...
from vli.takeoffs import load_takeoffs,recommend_takeoffs

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--config',default='config/locations.json'); ap.add_argument('--takeoffs',default='config/takeoffs.json'); ap.add_argument('--date'); ap.add_argument('--days',type=int,default=1); args=ap.parse_args()
    ...
    takeoffs=load_takeoffs(PROJECT_ROOT/args.takeoffs)
    ...
    for loc in cfg.locations:
        ...
        for off in range(days):
            ...
            result['takeoff_recommendations']=recommend_takeoffs(result.get('global',{}).get('direction_deg'),takeoffs)
